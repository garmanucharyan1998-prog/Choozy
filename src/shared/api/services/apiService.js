...
import {
  ...
  mockTopProducts,
  mockVarietyProducts,
} from "shared/api/mocks/mockData";
...
export const searchProducts = async (query, options = {}) => {
  ...
  if (options.brand) {
    filteredResults = filteredResults.filter(
      (item) => item.brand.toLowerCase() === options.brand.toLowerCase(),
    );
  }

  if (options.inStock !== undefined) {
    filteredResults = filteredResults.filter((item) => item.inStock === options.inStock);
  }
