      ...
      <div className="grid-catalog cont-width-default">
        {items.map((item) => (
          <button
            type="button"
            key={item.id}
            className={`${item.className} cursor-pointer border-0 bg-card-bg bg-contain bg-no-repeat bg-center rounded-xl sm:rounded-[20px] text-center flex items-end justify-center transition-all duration-[400ms] relative h-auto aspect-[16/10] md:hover:scale-[1.03] md:hover:bg-hover-blue xl:h-full xl:aspect-auto xl:bg-auto min-w-0 w-full p-0`}
